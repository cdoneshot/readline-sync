...
function _execFileSync(options, execOptions) {
  ...
  while (
    fs.readFileSync(pathDone, { encoding: options.encoding }).trim() !== '1'
...
function _readlineSync(options) {
  ...
  (function () {
    ...
    function getFsB() {
      if (!fsB) {
        ...
        // for v6.3.0+
        constants =
          constants && constants.fs && typeof constants.fs.O_RDWR === 'number'
            ? constants.fs
            : constants;
      }
      return fsB;
  ...
  (function () {
    ...
    while (true) {
      ...
      if (chunk) {
        // if input is del
        if (chunk == '\x7f' && input.length > 0) {
          if (options.mask != '') {
            fs.writeSync(fdW, '\b\x1b[0K');
          }
  ...
  if (typeof text !== 'string') {
    switch (param) {
      ...
      case 'limitCountNotZero':
        text =
          options[options.hasOwnProperty('limitSrc') ? 'limitSrc' : 'limit']
            .length;
      ...
      default:
        // with arg
        if (
          typeof (arg = (param.match(/^history_m(\d+)$/) || [])[1]) === 'string'
        ) {
        ...
        limit: function (value) {
          ...
          try {
            ...
            if (
              ...
              error =
                (exists ? 'Already exists' : 'No such file or directory') +
                ': ' +
                validPath;
              return false;
            }
